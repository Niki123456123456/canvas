...
use image::{ColorType, DynamicImage, ImageBuffer, ImageFormat, Rgb, Rgba, GenericImageView};
use resvg::FitTo;
use std::io::Cursor;
use usvg::{ScreenSize, ShapeRendering, TreeParsing, StrokeMiterlimit};
...
impl eframe::App for AmvApp {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui.button("auto").clicked() {
                        if let Some(state) = &self.state {
                            let crop = state.image.crop_imm(image.image_rect.min.x as _, image.image_rect.min.y as _, image.image_rect.width() as _, image.image_rect.height() as _,);
                            ...
                            let new_color = if diff_black> diff_white {Color32::BLACK} else { Color32::WHITE};
                            image.change_forgroundcolor([new_color.r(), new_color.g(), new_color.b()], ui);
                        }
                    }
                });
            }
        ...
        .frame(my_frame)
        .show(ctx, |ui| {
            let hover = ui.allocate_response(ctx.screen_rect().size(), Sense::click_and_drag()).hover_pos();
        ...
        egui::TopBottomPanel::bottom("my_panel").show(ctx, |ui| {
            let texture: &AppState = self.state.as_ref().unwrap();
